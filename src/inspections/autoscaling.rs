...
impl<'a> AutoscalingInspector<'a> {
    ...
    async fn inspect_hpas(&self, namespace: Option<&str>, issues: &mut Vec<Issue>) -> Result<CheckResult> {
        ...
        if hpas.items.is_empty() {
            return Ok(CheckResult {
                ...
                recommendations: vec!["Consider deploying HPAs to improve workload elasticity.".to_string()],
            });
        }

        let mut healthy = 0usize;
        for hpa in &hpas.items {
            let name = hpa.metadata.name.clone().unwrap_or_else(|| "unknown".to_string());
            ...
            if let Some(spec) = &hpa.spec {
                if spec.min_replicas.unwrap_or(1) == spec.max_replicas {
                    issues.push(Issue {
                        ...
                        recommendation: "Set a wider min/max replica range so the HPA can scale.".to_string(),
                ...
                if let Some(metrics) = &spec.metrics {
                    ...
                    issues.push(Issue {
                        ...
                        resource: Some(name.clone()),
                        recommendation: "Define CPU/Memory or custom metrics for this HPA.".to_string(),
                ...
                if let Some(behavior) = &spec.behavior {
                    ...
                    self.validate_behavior(behavior.scale_down.as_ref(), &name, "scale-down", issues);
                }
            }
    ...
    fn validate_behavior(&self, rules: Option<&HPAScalingRules>, name: &str, direction: &str, issues: &mut Vec<Issue>) {
            ...
            if let Some(select_policy) = &rules.select_policy {
                if select_policy.as_str() == "Disabled" {
                    issues.push(Issue {
                        ...
                        recommendation: "Review HPA behavior policy to ensure scaling is permitted when needed.".to_string(),
...
fn validate_target(target: &MetricTarget, metric_name: &str, hpa: &str, issues: &mut Vec<Issue>) {
    if target.average_utilization.is_none() && target.average_value.is_none() && target.value.is_none() {
        issues.push(Issue {
            severity: IssueSeverity::Warning,
            category: "Autoscaling".to_string(),
            description: format!("HPA {} metric {} missing scaling target", hpa, metric_name),
            resource: Some(hpa.to_string()),
            recommendation: "Configure averageUtilization, averageValue, or value for the metric target.".to_string(),
